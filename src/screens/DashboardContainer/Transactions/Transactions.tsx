              ...
              <>
                ...
                <View style={styles.filterView}>
                  ...
                  <Pressable  onPress={() => setDatePicker({to: false, from: true})} style={styles.datePickerBtn} >
                    <Text style={{fontFamily: 'Rubik-Regular', color: 'grey'}}>{filterCond.from?(new Date(filterCond.from).toISOString().slice(0,10)):''}</Text>
                  </Pressable>
                  <Text style={{color: 'black'}}>To : </Text>
                  <Pressable onPress={() => setDatePicker({to: true, from: false})} style={styles.datePickerBtn} >
                    <Text style={{fontFamily: 'Rubik-Regular', color: 'grey'}}>{filterCond.to?(new Date(filterCond.to).toISOString().slice(0,10)):''}</Text>
                  </Pressable>
                  <DatePicker
                    modal
